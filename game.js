...
function restartGame() {
    ...
    gameOver = false;
    player.setX(1500).setY(900).setVelocityX(0).setVelocityY(0);
    // Очистка бомб
    bombs.clear(true, true);
    // Відновлення платформ та інших об'єктів
    platforms.clear(true, true);
    objects.clear(true, true);

    // Додавання платформ та інших об'єктів
    for (var x = 0; x < worldWidth; x = x + 128) {
        platforms.create(x, 1080 - 128, 'ground').setOrigin(0, 0).refreshBody();
    }

    // Відновлення платформ, що знаходяться в небі
    for (var x = 0; x < worldWidth; x = x + Phaser.Math.Between(400, 500)) {
        var y = Phaser.Math.Between(100, 700)
    ...
    for (var x = 0; x <= worldWidth; x = x + Phaser.Math.Between(200, 800)) {
        objects.create(x, 987, 'crate').setScale(Phaser.Math.FloatBetween(0.5, 1,))
            .setDepth(Phaser.Math.Between(1, 3)).setOrigin(0, 1).refreshBody();
        objects.create(x, 987, 'stone').setScale(Phaser.Math.FloatBetween(0.5, 1,))
            .setDepth(Phaser.Math.Between(1, 3)).setOrigin(0, 1).refreshBody();
        objects.create(x, 989, 'tree').setScale(Phaser.Math.FloatBetween(0.5, 1,))
            .setDepth(Phaser.Math.Between(1, 3)).setOrigin(0, 1).refreshBody();
    }

    // Додавання колізії з гравцем
    this.physics.add.collider(player, platforms);
    ...
    // Додавання колізії з бомбами
    this.physics.add.collider(player, bombs, hitBomb, null, this);

    // Відновлення інших ігрових параметрів
    gameOver = false;
    player.setTint(); // Відміняємо забарвлення гравця
    this.physics.resume(); // Продовжуємо фізичний рух
